...
```sh
cargo build -p notra-core --release --example analyse_large_file_benchmark
```
...
`mmap` is currently benchmark-only. The generated fixture is owned by the benchmark and cannot change while mapped. Production documents can be modified or truncated by another process, so Notra keeps the safe owned read path until it has an immutable snapshot or a cross-platform file-stability contract.
