...
async function installAppUpdate() {
  ...
  appUpdateDetail = `正在准备下载 OtterDive ${pendingAppUpdate.version}`;
  renderAppUpdateStatus();
  ...
  try {
    await pendingAppUpdate.downloadAndInstall((event) => {
      if (event.event === "Started") {
        ...
        appUpdateDetail = `正在下载 OtterDive ${pendingAppUpdate?.version ?? "新版本"}`;
      } else if (event.event === "Progress") {
        downloadedBytes += event.data.chunkLength;
        appUpdateDetail = totalBytes > 0
          ? `正在下载 OtterDive ${pendingAppUpdate?.version ?? "新版本"} · ${Math.min(100, Math.round(downloadedBytes / totalBytes * 100))}%`
          : `正在下载 OtterDive ${pendingAppUpdate?.version ?? "新版本"}`;
      } else {
...
function renderAppUpdateStatus() {
  ...
  if (pendingAppUpdate) {
    ...
    $("appUpdatePopoverVersion").textContent = `OtterDive ${pendingAppUpdate.version} · 当前 ${pendingAppUpdate.currentVersion}`;
    $("appUpdatePopoverDetail").textContent = appUpdateStatus === "installing"
      ? appUpdateDetail
      : appUpdateStatus === "failed"
        ? "下载安装失败，请重试或稍后从设置中操作。"
        : releaseNote || "有新的 OtterDive 版本可用。";
...
async function refreshShellIntegrationStatus() {
  try {
    ...
    state.shellIntegration = integrationErrorStatus("以 OtterDive 打开", "读取右键菜单状态", error);
  } finally {
...
async function syncSystemIntegrationPreferences() {
  ...
  state.shellIntegration = contextMenu.status === "fulfilled"
    ? contextMenu.value
    : integrationErrorStatus("以 OtterDive 打开", "同步右键菜单", contextMenu.reason);
...
async function toggleShellIntegration() {
  ...
  try {
    ...
    scheduleSessionSave();
    log(enabled ? "已添加“以 OtterDive 打开”右键菜单" : "已移除“以 OtterDive 打开”右键菜单");
  } catch (error) {
...
async function toggleDefaultAppCandidate() {
  ...
  try {
    ...
    scheduleSessionSave();
    log(enabled ? "已注册 OtterDive 默认应用候选" : "已移除 OtterDive 默认应用候选");
  } catch (error) {
...
function refreshMarkdownResources(root: ParentNode, doc: OpenDocument) {
  root.querySelectorAll<HTMLImageElement>("img").forEach((image) => {
    ...
    const stored = image.dataset.otterdiveSource;
    ...
    if (!converted) return;
    image.dataset.otterdiveSource = source;
...
function renderAnalyseBookmarkDecorations() {
  ...
  analyseBookmarkDecorations.set(lines.map((line) => ({
    ...
    options: {
      isWholeLine: true,
      linesDecorationsClassName: "otterdive-analyse-bookmark-glyph",
      overviewRuler: {
...
async function exportKeybindings() {
  const path = await invoke<string | null>("pick_save_path", {
    request: { defaultDir: preferredDialogDirectory(), fileName: "otterdive-keybindings.json" },
  });
  if (!path) return;
...
function registerFormattingProviders() {
  monaco.languages.registerDocumentFormattingEditProvider("sql", {
    displayName: "OtterDive SQL Formatter",
...
function defineThemes() {
  ...
  monaco.editor.defineTheme("otterdive-light", {
    base: "vs",
    inherit: true,
    rules: [
  ...
  monaco.editor.defineTheme("otterdive-dark", {
    base: "vs-dark",
    inherit: true,
    rules: [
