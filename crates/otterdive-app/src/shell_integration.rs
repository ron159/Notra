...
const SHELL_MENU_LABEL: &str = "以 OtterDive 打开";
...
mod platform {
    ...
    const MANAGED_VALUE: &str = "OtterDiveManaged";
    const MANAGED_ID: &str = "dev.otterdive.app";

    const SHELL_KEY: &str = r"Software\Classes\*\shell\OtterDive.OpenWith";
    const SHELL_COMMAND_KEY: &str = r"Software\Classes\*\shell\OtterDive.OpenWith\command";

    const REGISTERED_APPLICATIONS_KEY: &str = r"Software\RegisteredApplications";
    const REGISTERED_APPLICATION_VALUE: &str = "OtterDive";
    const APPLICATION_KEY: &str = r"Software\Classes\Applications\otterdive.exe";
    const APPLICATION_COMMAND_KEY: &str =
        r"Software\Classes\Applications\otterdive.exe\shell\open\command";
    const SUPPORTED_TYPES_KEY: &str = r"Software\Classes\Applications\otterdive.exe\SupportedTypes";
    const CAPABILITIES_KEY: &str = r"Software\Classes\Applications\otterdive.exe\Capabilities";
    const FILE_ASSOCIATIONS_KEY: &str =
        r"Software\Classes\Applications\otterdive.exe\Capabilities\FileAssociations";
    const PROGID: &str = "OtterDive.Document";
    const PROGID_KEY: &str = r"Software\Classes\OtterDive.Document";
    const PROGID_ICON_KEY: &str = r"Software\Classes\OtterDive.Document\DefaultIcon";
    const PROGID_COMMAND_KEY: &str = r"Software\Classes\OtterDive.Document\shell\open\command";
    ...
    pub fn context_menu_status() -> Result<ShellIntegrationStatus, String> {
        ...
        if managed_id(&shell_key) != MANAGED_ID {
            return Ok(ShellIntegrationStatus {
                ...
                detail: "检测到同名的非 OtterDive 菜单项，未进行修改".to_owned(),
            });
        }
        let command = hkcu
    ...
    pub fn default_app_status() -> Result<ShellIntegrationStatus, String> {
        ...
        if managed_id(&application) != MANAGED_ID {
            return Ok(ShellIntegrationStatus {
                ...
                enabled: false,
                label: DEFAULT_APP_LABEL.to_owned(),
                detail: "默认应用候选信息不属于 OtterDive，未进行修改".to_owned(),
            });
        }
        let command = hkcu
    ...
    pub fn set_default_app_enabled(enabled: bool) -> Result<ShellIntegrationStatus, String> {
        ...
        if enabled {
            ...
            ensure_owned_or_missing(&hkcu, PROGID_KEY, "OtterDive 文档类型")?;
            ...
            application
                .set_value("FriendlyAppName", &"OtterDive")
            ...
            capabilities
                .set_value("ApplicationName", &"OtterDive")
                .and_then(|_| {
                    capabilities.set_value(
                        "ApplicationDescription",
                        &"OtterDive 文本、Markdown 与代码编辑器",
                    )
                })
            ...
            let (progid, _) = hkcu
                ...
                .map_err(|error| registry_error("创建 OtterDive 文档类型", error))?;
            progid
                .set_value("", &"OtterDive 文档")
                .and_then(|_| progid.set_value(MANAGED_VALUE, &MANAGED_ID))
                .map_err(|error| registry_error("写入 OtterDive 文档类型", error))?;
            let (progid_icon, _) = hkcu
                .create_subkey(PROGID_ICON_KEY)
                .map_err(|error| registry_error("创建 OtterDive 文档图标", error))?;
            progid_icon
                .set_value("", &icon)
                .map_err(|error| registry_error("写入 OtterDive 文档图标", error))?;
            let (progid_command, _) = hkcu
                .create_subkey(PROGID_COMMAND_KEY)
                .map_err(|error| registry_error("创建 OtterDive 文档打开命令", error))?;
            progid_command
                .set_value("", &command)
                .map_err(|error| registry_error("写入 OtterDive 文档打开命令", error))?;
            ...
            registered_apps
                ...
                .map_err(|error| registry_error("注册 OtterDive 默认应用候选", error))?;
        } else {
            if let Ok(registered_apps) = hkcu.open_subkey_with_flags(
                ...
                match registered_apps.get_value::<String, _>(REGISTERED_APPLICATION_VALUE) {
                    ...
                    Ok(_) => return Err("同名默认应用候选不属于 OtterDive，未执行删除".to_owned()),
                    Err(error) if error.kind() == io::ErrorKind::NotFound => {}
    ...
    fn current_executable() -> Result<PathBuf, String> {
        std::env::current_exe().map_err(|error| format!("无法获取 OtterDive 程序路径：{error}"))
    }
    ...
    fn ensure_owned_or_missing(hkcu: &RegKey, path: &str, label: &str) -> Result<(), String> {
        match hkcu.open_subkey(path) {
            Ok(existing) if managed_id(&existing) != MANAGED_ID => {
                Err(format!("存在同名的非 OtterDive {label}，无法安全覆盖"))
            }
            Ok(_) => Ok(()),
    ...
    fn delete_owned_key(hkcu: &RegKey, path: &str, label: &str) -> Result<(), String> {
        match hkcu.open_subkey(path) {
            Ok(existing) if managed_id(&existing) != MANAGED_ID => {
                Err(format!("同名{label}不属于 OtterDive，未执行删除"))
            }
            Ok(_) => hkcu
    ...
    pub(super) fn remove_candidate_progid_for_test() {
        let hkcu = RegKey::predef(HKEY_CURRENT_USER);
        let _ = delete_owned_key(&hkcu, PROGID_KEY, "OtterDive 文档类型");
    }
}
...
mod tests {
    ...
    fn shell_command_quotes_executable_and_file_placeholder() {
        let executable = PathBuf::from(r"C:\Program Files\OtterDive\otterdive.exe");
        assert_eq!(
            shell_command(&executable),
            r#""C:\Program Files\OtterDive\otterdive.exe" "%1""#
        );
    }
